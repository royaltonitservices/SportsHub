...
// MARK: - Block disclosure copy (testable, must match ACTUAL enforcement)

/// The user-facing explanation shown before a block. Kept here (not inline in the view) so a
/// regression test can assert it promises ONLY what the backend actually enforces TODAY:
///   • direct messages + friend requests between the two users are stopped (`is_blocked` on DM
///     send/conversation + friend request/accept), and
///   • the blocked person's messages are hidden from the blocker's view of shared GROUP CHATS
///     (`get_group_messages` / `get_user_groups` filter blocked-author messages).
/// It deliberately does NOT claim content is deleted, that the person disappears everywhere, or
/// that their posts/clips/profile are hidden — broader blocked-content visibility is a later slice.
enum BlockDisclosure {
    static let message =
        "Blocking stops direct messages and friend requests between you, and hides their messages "
        + "in group chats you both belong to. It won't delete existing messages, and you may still "
        + "see their posts, clips, or profile elsewhere in the app."
}
...
struct UserSafetyMenu<Extra: View>: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("Block @\(username)?",
            ...
            Text(BlockDisclosure.message + " You can report them first if you'd like.")
        }
...
// MARK: - Previews (dev-only runtime render proof of the 1.4E safety surfaces)

#Preview("Report comment (content)") {
    ReportContentView(contentType: "comment", contentId: "demo-comment",
                      contentPreview: "you played like garbage, quit the team")
}

#Preview("Report account (user)") {
    ReportContentView(contentType: "user", contentId: "demo-user", contentPreview: "@samplerival")
}

#Preview("Block disclosure copy") {
    VStack(alignment: .leading, spacing: 16) {
        Text("Block @samplerival?").font(.headline)
        Text(BlockDisclosure.message + " You can report them first if you'd like.")
            .font(.callout).foregroundStyle(.secondary)
        HStack {
            Button("Report Instead…") { }
            Spacer()
            Button("Block", role: .destructive) { }
        }
    }
