...
// MARK: - Block disclosure honesty (copy must match actual enforcement)

@Suite("BlockDisclosure honesty")
struct BlockDisclosureTests {
    @Test("promises the two behaviors that ARE enforced today")
    func promisesEnforced() {
        let m = BlockDisclosure.message.lowercased()
        #expect(m.contains("direct messages"))           // is_blocked on DM send/conversation
        #expect(m.contains("friend requests"))           // is_blocked on friend request/accept
        #expect(m.contains("group chats"))               // group-message suppression is enforced
        #expect(m.contains("won't delete") || m.contains("not delete"))  // history retained
    }

    @Test("does NOT over-claim beyond current enforcement")
    func noOverClaim() {
        let m = BlockDisclosure.message.lowercased()
        // Broader blocked-content visibility is a LATER slice, so the copy must acknowledge that
        // posts/clips/profile remain visible and must not imply total disappearance or deletion.
        #expect(m.contains("posts") && m.contains("clips") && m.contains("profile"))
        #expect(!m.contains("everywhere"))
        #expect(!m.contains("delete their account"))
        #expect(!m.contains("hides everything"))
    }
}
